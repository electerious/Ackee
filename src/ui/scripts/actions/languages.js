...
export const SET_LANGUAGES_RANGE = Symbol()
...
export const setLanguagesRange = (payload) => ({
	type: SET_LANGUAGES_RANGE,
	payload
})
...
export const fetchLanguages = signalHandler((signal) => (props, domainId) => async (dispatch) => {
	...
	try {
		...
		const data = await api(`/domains/${ domainId }/languages?sorting=${ props.languages.sorting }&range=${ props.languages.range }`, {
			method: 'get',
			props,
